package com.dtflys.forest.test.exception;
...
public class TestExceptions {
    ...
    public void testInterceptorDefineException() {
        ...
        assertThat(exception.getMessage())
                .isEqualTo("[Forest] Interceptor class 'com.dtflys.forest.test.exception.TestExceptions$TestErrorInterceptor' cannot be initialized, because interceptor class must implements com.dtflys.forest.interceptor.Interceptor");
    ...
    public interface TestClient {
        ...
        @Get(url = "/data/")
        String fileVar(@DataFile(value = "file", fileName = "dir/{filename}") String h);
    }
    ...
    public void testVariableUndefinedException3() {
        ...
        exception.printStackTrace();
        assertThat(exception).isNotNull().isInstanceOf(ForestVariableUndefinedException.class);
        assertThat(exception.getMessage().replaceAll("\\[\\d(\\d)?m", "")).isEqualTo("Cannot resolve variable 'json'\n" +
                "\n" +
                "\t[From Template]\n" +
                "\n" +
                "\tmethod: com.dtflys.forest.test.exception.TestExceptions$TestClient#contentTypeVar()\n" +
                "\tannotation: com.dtflys.forest.annotation.@Get\n" +
                "\tattribute: contentType = \"application/{json}\"\n" +
                "\t                                       ^^^^ Cannot resolve variable 'json'\n");
    }
    ...
    public void testVariableUndefinedException_file() {
        ...
        assertThat(exception).isNotNull().isInstanceOf(ForestVariableUndefinedException.class);
        exception.printStackTrace();
        assertThat(exception.getMessage().replaceAll("\\[\\d(\\d)?m", "")).isEqualTo("Cannot resolve variable 'filename'\n" +
                "\n" +
                "\t[From Template]\n" +
                "\n" +
                "\tmethod: com.dtflys.forest.test.exception.TestExceptions$TestClient#fileVar(java.lang.String)\n" +
                "\tannotation: com.dtflys.forest.annotation.@DataFile\n" +
                "\tattribute: fileName = \"dir/{filename}\"\n" +
                "\t                            ^^^^^^^^ Cannot resolve variable 'filename'\n");
    }
