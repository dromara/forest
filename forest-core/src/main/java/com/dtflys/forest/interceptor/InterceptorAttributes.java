...
public class InterceptorAttributes {
    ...
    public Map<String, Object> render(ForestRequest request, Object[] args) {
        for (Map.Entry<String, Object> entry : attributeTemplates.entrySet()) {
            final String name = entry.getKey();
            Object value = entry.getValue();
            if (value instanceof MappingTemplate) {
                value = ((MappingTemplate) value).render(request, args);
                ...
                for (int i = 0; i < len; i++) {
                    strArray[i] = templates[i].render(request, args);
                }
                value = strArray;
            }
