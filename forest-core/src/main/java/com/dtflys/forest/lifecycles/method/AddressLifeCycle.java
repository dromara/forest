...
public class AddressLifeCycle implements MethodAnnotationLifeCycle<Address> {
    ...
    public void onInvokeMethod(ForestRequest request, ForestMethod method, Object[] args) {
        ...
        if (StringUtils.isNotBlank(basePathStr)) {
            final MappingTemplate basePathTemplate = MappingTemplate.annotation(Address.class, "basePath", basePathStr.trim());
            basePath = basePathTemplate.render(request);
        }

        // 判断是否有设置 scheme
        if (StringUtils.isNotBlank(schemeStr)) {
            final MappingTemplate schemeTemplate = MappingTemplate.annotation(Address.class, "schema", schemeStr.trim());
            scheme = schemeTemplate.render(request);
        }

        // 判断是否有设置 host
        if (StringUtils.isNotBlank(hostStr)) {
            final MappingTemplate hostTemplate = MappingTemplate.annotation(Address.class, "host", hostStr.trim());
            host = hostTemplate.render(request);
        }

        // 判断是否有设置 port
        if (StringUtils.isNotBlank(portStr)) {
            final MappingTemplate portTemplate = MappingTemplate.annotation(Address.class, "port", portStr.trim());
