...
public class ForestMethod<T> implements VariableScope {
    ...
    private MappingURLTemplate makeURLTemplate(
            ...
            final String attributeName,
            final String text) {
        return MappingURLTemplate.annotation(annotationType, attributeName, text);
    }
    ...
    private MappingTemplate getOrCreateTemplate(
            ...
            final String attributeName,
            final String text) {
        return MappingTemplate.annotation(annotationType, attributeName, text);
    }
     ...
     * 获得最终的请求类型
     *
     * @param request Forest 请求
     * @return 请求类型，{@link ForestRequestType}枚举实例
     */
    private ForestRequestType type(final ForestRequest<?> request) {
        final String renderedType = typeTemplate.render(request);
        if (StringUtils.isBlank(renderedType)) {
            final String typeFromName = methodNameItems[0];
            final ForestRequestType type = ForestRequestType.findType(typeFromName);
            if (type != null) {
                return type;
            }
            return ForestRequestType.GET;
        }
        final ForestRequestType type = ForestRequestType.findType(renderedType);
        if (type != null) {
            return type;
        }
    ...
    private ForestRequest<?> makeRequest(Object[] args) {
        ...
        final ForestRequestType type = type(request);

        if (baseUrlTemplate != null) {
            baseURL = baseUrlTemplate.renderURL(request);
        }
        if (urlTemplate == null) {
        ...
        if (baseEncodeTemplate != null) {
            baseContentEncoding = baseEncodeTemplate.render(request);
        }
        String contentEncoding = null;
        if (encodeTemplate != null) {
            contentEncoding = encodeTemplate.render(request);
        }

        String baseContentType = null;
        if (baseContentTypeTemplate != null) {
            baseContentType = baseContentTypeTemplate.render(request);
        }
        String baseUserAgent = null;
        if (baseUserAgentTemplate != null) {
            baseUserAgent = baseUserAgentTemplate.render(request);
        }
        String charset = null;
        String renderedCharset = charsetTemplate.render(request);
        if (StringUtils.isNotBlank(renderedCharset)) {
            charset = renderedCharset;
        } else if (baseCharsetTemplate != null) {
            charset = baseCharsetTemplate.render(request);
        }

        String responseEncoding = null;
        if (responseEncodingTemplate != null) {
            responseEncoding = responseEncodingTemplate.render(request);
        }

        String sslProtocol = null;
        String renderedSslProtocol = sslProtocolTemplate.render(request);
        if (StringUtils.isNotBlank(renderedSslProtocol) && !"null".equals(renderedSslProtocol)) {
            ...
            sslProtocol = baseSslProtocolTemplate.render(request);
        } else {
        ...
        if (contentTypeTemplate != null) {
            renderedContentType = contentTypeTemplate.render(request).trim();
        }
        String renderedUserAgent = null;
        if (userAgentTemplate != null) {
            renderedUserAgent = userAgentTemplate.render(request).trim();
        }
...
*/
        ...
        if (bodyTypeTemplate != null) {
            bodyTypeName = bodyTypeTemplate.render(request);
        }
            ...
            if (parameter.isObjectProperties()) {
                ...
                if (parameter.isJsonParam()) {
                    ...
                    if (MappingParameter.isHeader(target)) {
                        request.header(new RequestNameValue(parameter.getJsonParamName(), json, target)
                    ...
                    if (obj != null) {
                        if (MappingParameter.isHeader(target)) {
                            request.header(new RequestNameValue(null, obj, target));
                        } else if (MappingParameter.isQuery(target)) {
                            request.query(obj.toString(), (Object) null,
                    ...
                    if (MappingParameter.isQuery(target)) {
                        request.query(ForestQueryParameter.createSimpleQueryParameter(request.getQuery(), obj)
                                .setDefaultValue(parameter.getDefaultValue()));
                    } else if (MappingParameter.isBody(target)) {
                        request.body(new StringRequestBody(obj.toString())
                                .setDefaultValue(parameter.getDefaultValue()));
                    }
                } else if (obj instanceof Map) {
                    Map map = (Map) obj;
                    if (MappingParameter.isQuery(target)) {
                        request.query(map, parameter.isUrlEncode(), parameter.getCharset());
                    } else if (MappingParameter.isBody(target)) {
                        request.body(map, parameter.getPartContentType());
                    ...
                    if (MappingParameter.isQuery(target)) {
                        if (parameter.isJsonParam()) {
                            request.query(parameter.getName(), obj,
                                    parameter.isUrlEncode(), parameter.getCharset());
                        } else {
                            if (obj instanceof Iterable) {
                                for (Object subItem : (Iterable) obj) {
                                    if (subItem instanceof SimpleQueryParameter) {
                                        request.query((SimpleQueryParameter) subItem);
                                    } else {
                                        request.query(ForestQueryParameter.createSimpleQueryParameter(
                                                request.getQuery(), subItem));
                                    }
                                }
                            } else if (obj.getClass().isArray()) {
                                if (obj instanceof SimpleQueryParameter[]) {
                                    request.query((SimpleQueryParameter[]) obj);
                                }
                            }
                        }
                    } else if (MappingParameter.isBody(target)) {
                    ...
                    if (MappingParameter.isQuery(target) &&
                        ...
                        if (MappingParameter.isHeader(target)) {
                            ...
                            if (!parameter.isJsonParam() && obj instanceof Iterable) {
                                ...
                                request.jsonQuery(parameter.getName(), obj);
                            } else {
                                request.query(
        ...
        for (int i = 0; i < multipartFactories.size(); i++) {
            ...
            factory.addMultipart(context, nameTemplate, fileNameTemplate, data, multiparts, args);
        }
        request.multipartList(multiparts);
        // setup ssl keystore
        if (sslKeyStoreId != null) {
            SSLKeyStore sslKeyStore = null;
            String keyStoreId = sslKeyStoreId.render(request);
        ...
        if (configuration.getDefaultParameters() != null) {
            request.nameValueList(configuration.getDefaultParameters());
        }
        if (baseHeaders != null && baseHeaders.length > 0) {
            HeaderUtils.addHeaders(request, request, baseHeaders, args);
        }
        if (configuration.getDefaultHeaders() != null) {
            request.headers(configuration.getDefaultHeaders());
        }
        ...
        if (renderedContentType == null || renderedContentType.equalsIgnoreCase(ContentType.APPLICATION_X_WWW_FORM_URLENCODED)) {
            for (int i = 0; i < dataTemplateArray.length; i++) {
                ...
                String data = dataTemplate.render(request);
        ...
        } else if (configuration.connectTimeout() != null) {
        ...
        } else if (configuration.readTimeout() != null) {
        ...
        } else if (configuration.maxRetryCount() != null) {
            request.setMaxRetryCount(configuration.maxRetryCount());
        }
        ...
        } else if (configuration.maxRetryInterval() >= 0) {
            request.setMaxRetryInterval(configuration.maxRetryInterval());
        }

        Class globalRetryerClass = configuration.retryer();
        ...
        String dataType = dataTypeTemplate.render(request);
        ...
        if (interceptorAttributesList != null && interceptorAttributesList.size() > 0) {
            for (InterceptorAttributes attributes : interceptorAttributesList) {
                ...
                request.interceptorAttributes(newAttrs.getInterceptorClass(), newAttrs);
                request.getInterceptorAttributes(newAttrs.getInterceptorClass()).render(context);
            }
        }

        if (globalInterceptorList != null && globalInterceptorList.size() > 0) {
            for (Interceptor item : globalInterceptorList) {
                request.interceptor(item);
            }
        }
